function [expMin,stdMin,expMax,stdMax] = apm_minMaxDoseProb(expDose,covDose,method)
%UNTITLED7 Summary of this function goes here
%   Detailed explanation goes here

if nargin < 3
    method = 'taylor_genMean';
end
...
switch method
    case 'taylor_genMean'
        [expMin,stdMin] = apm_eudProb(expDose,covDose,-k,'taylor_direct');
        if nargout > 2
            [expMax,stdMax] = apm_eudProb(expDose,covDose,k,'taylor_direct');
        end

    case 'taylor_smoothMaxExpSum'
        nFac = 1/max(expDose(:));
        expDose = expDose * nFac;
        covDose = covDose * nFac^2;
                
        gradFunc = @(x,a) exp(a*transpose(x)) ./ sum(exp(a*x)) .* (1 + a*(transpose(x) - apm_smoothMaxDoseExpSum(x,a))); 
        [expMin,stdMin] = apm_taylorUncertaintyPropagation(expDose,covDose,@(x) apm_smoothMaxDoseExpSum(x,-k),@(x) gradFunc(x,-k));
        if nargout > 2
            [expMax,stdMax] = apm_taylorUncertaintyPropagation(expDose,covDose,@(x) apm_smoothMaxDoseExpSum(x,k),@(x) gradFunc(x,k));
        end
        ...
        expMax = expMax / nFac;
        stdMax = stdMax / nFac;
    case 'taylor_smoothMaxLogExp'
        gradFunc = @(x) exp(transpose(x)) ./ (sum(x) - numel(x) + 1);
        [expMin,stdMin] = apm_taylorUncertaintyPropagation(expDose,covDose,@(x) -apm_smoothMaxDoseLogExp(-x),@(x) -gradFunc(-x));
        if nargout > 2
            [expMax,stdMax] = apm_taylorUncertaintyPropagation(expDose,covDose,@(x) apm_smoothMaxDoseLogExp(x),@(x) gradFunc(x));
        end
    case 'iter'
        [expMin,stdMin] = maxgauss(-expDose,covDose);
        expMin = -expMin;
        stdMin = sqrt(stdMin);
        if nargout > 2
            [expMax,stdMax] = maxgauss(expDose,covDose);
            stdMax = sqrt(stdMax);
        end
    otherwise
...
%found on https://github.com/sg3510/ASP4/blob/master/Matlab/VB/maxgauss.m
...
while nm>1
    ...
    ix=ix-floor(ix/nm);
    nm=nm-1;
    matRad_progress(length(m)-nm,length(m));
end         % main while loop
u=m(1);
v=c(1);
